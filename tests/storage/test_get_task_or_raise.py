...
async def test_get_task_or_raise_happy_path(fixture_todolist: schemes.Todo):
    exist_task = await storage.todolist.upsert_task(fixture_todolist.uid, title='test 1')

    response = await storage.todolist.get_task_or_raise(exist_task.uid)
...
async def test_get_task_or_raise_not_found(fixture_todolist: schemes.Todo):
    with pytest.raises(RuntimeError):
        await storage.todolist.get_task_or_raise('invalid-task-uid')
