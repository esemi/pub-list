...
async def test_upsert_task_insert(fixture_todolist: schemes.Todo):
    response = await storage.upsert_task(fixture_todolist.uid, 'first title', None)
...
async def test_upsert_task_update(fixture_todolist: schemes.Todo):
    exist_task = await storage.upsert_task(fixture_todolist.uid, 'first title', None, 123)

    response = await storage.upsert_task(fixture_todolist.uid, 'updated title', task_uid=exist_task.uid)
